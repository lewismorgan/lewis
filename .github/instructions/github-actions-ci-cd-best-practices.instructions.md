...
- **Deeper Dive:**
  ...
  - **Cache Keys:** Use a unique key based on file hashes (e.g., `hashFiles('**/pnpm-lock.json')`, `hashFiles('**/requirements.txt')`) to invalidate the cache only when dependencies change.
...
- name: Cache Node.js modules
  ...
  with:
    ...
    key: ${{ runner.os }}-node-${{ hashFiles('**/pnpm-lock.json') }}-${{ github.run_id }}
    restore-keys: |
      ${{ runner.os }}-node-${{ hashFiles('**/pnpm-lock.json') }}-
      ${{ runner.os }}-node-
```
...
- **Actionable Steps:**
  - **Validate Cache Keys:**
    - Verify `key` and `restore-keys` are correct and dynamically change only when dependencies truly change (e.g., `key: ${{ runner.os }}-node-${{ hashFiles('**/pnpm-lock.json') }}`). A cache key that is too dynamic will always result in a miss.
